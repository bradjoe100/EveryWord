...
public class Prompt
{
	...
	public static char getChar (String ask)
	{
		...
		boolean found = false;
		while (! found) {
			String stringInput = getString(ask);
			if (stringInput.length() == 1) {
				input = stringInput.charAt(0);
				found = true;
			}
			else {
				found = false;
	...
	/**
	 *	Prompts the user for a character in a string of specific charaters,
	 * 	and returns the character.
	 *	@param ask  		The prompt line
	 * 	@param charFilter	The array that contains the accepted characters
	 *	@return  			The character input
	 */
	public static char getChar (String ask, String charFilter)
	{
		char input = ' ';
		boolean found = false;
		while (! found) {
